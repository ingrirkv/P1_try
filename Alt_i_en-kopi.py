...
model.alpha = pyo.Var(model.Iteration, domian = pyo.NonNegativeReals) #usikker på om alpha er en variabel
#master
...
#SubProblem
def SubProblem(V1_t[24]): #må finne ut
model2 = pyo.ConcreteModel()
    ...
    def constraint_dual(model2,t):
        if (t==25):
             return (model2.V1_t[24] == model2.x_1) #legger til at output volum fra masterproblem er nå input volum i sub problem
        else:
